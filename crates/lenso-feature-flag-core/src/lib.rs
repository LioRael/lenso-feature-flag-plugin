//! Shared Feature Flag authorization, request policy and deterministic domain logic.
pub mod domain;
pub mod store;
use lenso::prelude::*;
use lenso_auth_sdk::{
...
use crate::domain::{DomainFailure, StorageError};
...
impl PolicyConfig {
    pub fn validate(&self) -> Result<(), FeatureFlagConfigError> {
    ...
    pub fn verifier(&self) -> Result<ActorAssertionVerifier, RuntimeFailure> {
        ActorAssertionVerifier::from_public_key_base64(
            self.auth_issuer.clone(),
            &self.auth_assertion_public_key,
        )
...
pub struct FeatureFlags<S> {
    ...
    pub membership: Port<membership::OrganizationMembershipClient>,
    pub access: Port<access::AccessControlClient>,
    pub storage: S,
}
impl<S: crate::store::Store> FeatureFlags<S> {
    async fn authorize(
        &self,
        context: &Ctx,
        callers: &[String],
    ...
    pub async fn get_flag(
        ...
        let record = map_storage(
            self.storage
                .get_flag(&request.organization_id, &request.flag_key)
                .await,
    ...
    pub async fn list_evaluation_receipts(
        &self,
        context: Ctx,
...
fn ruleset_definition(
    request: &admin::PublishRulesetRequest,
) -> Result<domain::RulesetDefinition, ()> {
...
) -> Result<domain::ValueRecord, ()> {
    Ok(domain::ValueRecord {
...
fn command<'a>(auth: &'a Authorized, key: &'a str, hash: &'a [u8]) -> domain::Command<'a> {
    domain::Command {
        caller: &auth.caller,
        actor: &auth.actor,
        key,
